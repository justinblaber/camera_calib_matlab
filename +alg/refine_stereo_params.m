function [params, cov_params] = refine_stereo_params(params, p_cb_ws, p_cb_p_dss, idx_valids, f_p_cb_w2p_cb_p, f_dp_cb_p_dh, f_p_p2p_p_d, f_dp_p_d_dargs, optimization_type, opts, cov_cb_p_dss)
    ...
    %           pixel points
    %       .R - cell; Nx1 cell array of calibration board distorted
    %           pixel points
    %   idx_valids - struct;
...
function [jacob, res] = calc_gauss_newton_params(params, p_cb_ws, p_cb_p_dss, idx_valids, f_p_cb_w2p_cb_p, f_dp_cb_p_dh, f_p_p2p_p_d, f_dp_p_d_dargs, idx_update)
    ...
    for i = 1:num_boards
        ...
        p_cb_p_d_m_Ls = alg.p_cb_w2p_cb_p_d(p_cb_ws, f_p_cb_w2p_cb_p, R_L, t_L, f_p_p2p_p_d, a_L, d_L);

        % Store residuals - take valid indices into account
        res_L(2*sum(vertcat(idx_valids.L{1:i-1}))+1:2*sum(vertcat(idx_valids.L{1:i}))) = ...
            reshape(vertcat((p_cb_p_d_m_Ls(idx_valids.L{i}, 1)-p_cb_p_dss.L{i}(idx_valids.L{i}, 1))', ...
                            (p_cb_p_d_m_Ls(idx_valids.L{i}, 2)-p_cb_p_dss.L{i}(idx_valids.L{i}, 2))'), [], 1);

        % Intrinsics
        jacob_L(2*sum(vertcat(idx_valids.L{1:i-1}))+1:2*sum(vertcat(idx_valids.L{1:i})), 1:3+num_params_d) = ...
            alg.dp_cb_p_d_dintrinsic(p_cb_ws(idx_valids.L{i}, :), f_p_cb_w2p_cb_p, f_dp_cb_p_dh, R_L, t_L, f_dp_p_d_dargs, a_L, d_L); %#ok<SPRIX>
        ...
        jacob_L(2*sum(vertcat(idx_valids.L{1:i-1}))+1:2*sum(vertcat(idx_valids.L{1:i})), 7+2*num_params_d+6*(i-1):12+2*num_params_d+6*(i-1)) = ...
            alg.dp_cb_p_d_dextrinsic(p_cb_ws(idx_valids.L{i}, :), f_p_cb_w2p_cb_p, f_dp_cb_p_dh, R_L, t_L, f_dp_p_d_dargs, a_L, d_L, drt_L_dm_L); %#ok<SPRIX>
    end
    ...
    for i = 1:num_boards
        ...
        % Get model calibration board distorted pixel points
        p_cb_p_d_m_Rs = alg.p_cb_w2p_cb_p_d(p_cb_ws, f_p_cb_w2p_cb_p, R_R, t_R, f_p_p2p_p_d, a_R, d_R);

        % Store residuals - take valid indices into account
        res_R(2*sum(vertcat(idx_valids.R{1:i-1}))+1:2*sum(vertcat(idx_valids.R{1:i}))) = ...
            reshape(vertcat((p_cb_p_d_m_Rs(idx_valids.R{i}, 1)-p_cb_p_dss.R{i}(idx_valids.R{i}, 1))', ...
                            (p_cb_p_d_m_Rs(idx_valids.R{i}, 2)-p_cb_p_dss.R{i}(idx_valids.R{i}, 2))'), [], 1);

        % Intrinsics
        jacob_R(2*sum(vertcat(idx_valids.R{1:i-1}))+1:2*sum(vertcat(idx_valids.R{1:i})), 4+num_params_d:6+2*num_params_d) = ...
            alg.dp_cb_p_d_dintrinsic(p_cb_ws(idx_valids.R{i}, :), f_p_cb_w2p_cb_p, f_dp_cb_p_dh, R_R, t_R, f_dp_p_d_dargs, a_R, d_R); %#ok<SPRIX>
        ...
        jacob_R(2*sum(vertcat(idx_valids.R{1:i-1}))+1:2*sum(vertcat(idx_valids.R{1:i})), 7+2*num_params_d+6*(i-1):12+2*num_params_d+6*(i-1)) = ...
            alg.dp_cb_p_d_dextrinsic(p_cb_ws(idx_valids.R{i}, :), f_p_cb_w2p_cb_p, f_dp_cb_p_dh, R_R, t_R, f_dp_p_d_dargs, a_R, d_R, drt_R_dm_L); %#ok<SPRIX>
        ...
        jacob_R(2*sum(vertcat(idx_valids.R{1:i-1}))+1:2*sum(vertcat(idx_valids.R{1:i})), 7+2*num_params_d+6*num_boards:12+2*num_params_d+6*num_boards) = ...
            alg.dp_cb_p_d_dextrinsic(p_cb_ws(idx_valids.R{i}, :), f_p_cb_w2p_cb_p, f_dp_cb_p_dh, R_R, t_R, f_dp_p_d_dargs, a_R, d_R, drt_R_dm_s); %#ok<SPRIX>
    end
