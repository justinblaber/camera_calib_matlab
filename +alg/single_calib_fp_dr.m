function calib = single_calib_fp_dr(img_cbs,p_fp_p_dss,calib_config,intrin)
    ...
    f_p_n2p_n_d = matlabFunction(calib_config.sym_p_n2p_n_d);
    
    % Get function handles for distortion function partial derivatives
    args_p_n2p_n_d = argnames(calib_config.sym_p_n2p_n_d);
    for i = 1:length(args_p_n2p_n_d)
        % Differentiate
        f_dp_n_d_dargs{i} = diff(calib_config.sym_p_n2p_n_d, ...
                                 args_p_n2p_n_d(i)); %#ok<AGROW>
        % Convert to function handle 
        f_dp_n_d_dargs{i} = matlabFunction(f_dp_n_d_dargs{i}); %#ok<AGROW>
    end    
        
    % Get number of distortion params    
    num_params_d = nargin(f_p_n2p_n_d) - 2;
    ...
    for it = 1:distortion_refinement_it_cutoff
            ...
            for i = 1:num_boards
                ...
                if exist('A','var') && exist('d','var')
                    ...
                    p_fp_ps = alg.p_p_d2p_p(p_fp_p_dss{i}, ...
                                            ...
                                            f_p_n2p_n_d, ...
                                            f_dp_n_d_dargs{1}, ... % x_n
                                            f_dp_n_d_dargs{2}, ... % y_n                                            
                                            A, ...
                                            d, ...
                                            calib_config);
                else
        ...
        for i = 1:num_boards
            for j = 1:numel(cov_cb_pss{i})
                % Initialize updated covariance - this insures number of 
                ...
                if idx_valids{i}(j)
                    ...
                    % Get derivatives
                    dp_p_d_dp_p_bar = vertcat(alg.p_p2p_p_d(p_cb_p,f_dp_n_d_dargs{1},A,d), ... % x_p_bar
                                              alg.p_p2p_p_d(p_cb_p,f_dp_n_d_dargs{2},A,d));    % y_p_bar
        ...
        % Get transform to convert world points to ideal pixels
        switch calib_config.target_type
            case 'checker'
                % Use "point to point" method
