function calib = stereo_calib_H(f_single_calib_H, obj_A, obj_R, obj_cb_w2p, obj_distortion, obj_cb_geom, img_cbs, H_w2ps, calib_config, intrin)
    % Performs stereo camera calibration using "homography" method.
    %
    % Inputs:
    ...
    %   obj_cb_geom - lass.cb_geom.target_intf; calibration board
    %       four-point and target geometry interface.
    %   img_cbs - struct;
    %       .L - class.img.intf; Nx1 calibration board image interfaces.
    %       .R - class.img.intf; Nx1 calibration board image interfaces.
    %   H_w2ps - struct;
    %       .L - cell; Nx1 cell of initial guesses of homographies which
    %           map world coordinates to pixel coordinates
    %       .R - cell; Nx1 cell of initial guesses of homographies which
    %           map world coordinates to pixel coordinates
    ...
    if exist('intrin', 'var')
        calib_L = f_single_calib_H(obj_A, ...
                                   obj_R, ...
                                   obj_cb_w2p, ...
                                   obj_distortion, ...
                                   obj_cb_geom, ...
    ...
    if exist('intrin', 'var')
        calib_R = f_single_calib_H(obj_A, ...
                                   obj_R, ...
                                   obj_cb_w2p, ...
                                   obj_distortion, ...
                                   obj_cb_geom, ...
