function calib = stereo_calib_fp_dr(img_cbs,p_fp_p_dss,calib_config,intrin)      
    ...
    if exist('intrin','var')
        ...
        if intrin.L.A(1,1) ~= intrin.L.A(2,2) || any(intrin.L.A([2 3 4 6])) || ...
           intrin.R.A(1,1) ~= intrin.R.A(2,2) || any(intrin.R.A([2 3 4 6]))          
            error('Invalid initial camera matrix!')
        end
            
        % Validate d - make sure length of d is correct
        if size(intrin.L.d,1) ~= num_params_d || size(intrin.L.d,2) ~= 1 || ...
           size(intrin.R.d,1) ~= num_params_d || size(intrin.R.d,2) ~= 1     
