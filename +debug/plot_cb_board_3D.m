function plot_cb_board_3D(cal_config,xform,color,alpha,a)
    ...
    for j = 1:cal_config.num_squares_width
        for k = 1:cal_config.num_squares_height
            % Get checker color; only odd # of checkers are allowed on
            % each side, so this is ok
            if ~util.is_even((j-1)*cal_config.num_squares_height+k)
                patch_color = color;
            else
                patch_color = 'white';
            end            
            ...
            % Get checker coords
            x_w_cb = [(j-1)*cal_config.square_size (j-1)*cal_config.square_size ...
                      j*cal_config.square_size j*cal_config.square_size]+width_offset;
            y_w_cb = [(k-1)*cal_config.square_size k*cal_config.square_size ...
                      k*cal_config.square_size (k-1)*cal_config.square_size]+height_offset;
