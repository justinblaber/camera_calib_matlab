...
%{
    ...
    e2 = ezplot(@(x,y) l1(1).*x + l1(2).*y + l1(3),[1 size(array,2) 1 size(array,1)]);
    e3 = ezplot(@(x,y) l2(1).*x + l2(2).*y + l2(3),[1 size(array,2) 1 size(array,1)]);
    set(e1,'color','r');
    set(e2,'color','g');
    set(e3,'color','g');
%}
    ...
    assert(all(all(abs(l1 - [-0.003623782028800;
                             -0.000196862044249;
                              0.112365384116166]) < eps('single'))));
    assert(all(all(abs(l2 - 1.0e+03 * [ 0.001000000000000;
                                       -0.069586340539404
                                        2.225079029539678]) < eps('single'))));
end