function test_refine_checker_edges
    ...
    % Assert
    assert(all(all(abs(p - [15.600192089575570  16.260506288011388]) < 1e-4)));
    assert(all(all(abs(cov_p - 1.0e-04 * [ 0.601856069578277  -0.044299438511393
                                          -0.044299438511393   0.593612470227110]) < 1e-4)));
end
